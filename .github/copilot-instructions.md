...
The project uses a custom event system (`infra/events`) to track all user actions, biometric captures, matches, and workflow steps as structured, auditable events.
...
* Feature modules create events in use cases (e.g., `AddCaptureEventsUseCase`) and call the repository to persist them.
