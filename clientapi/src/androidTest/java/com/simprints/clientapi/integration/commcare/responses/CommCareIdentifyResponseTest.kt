...
class CommCareIdentifyResponseTest: BaseClientApiTest() {
    ...
    private val matchResults = listOf(
    ...
    fun appModuleSendsAnIdentifyAppResponse_shouldReturnACommCareIdentifyResponse() {
        val appIdentifyResponse = AppIdentifyResponse(matchResults, "session_id")
    ...
    private fun assertCommCareIdentifyResponse(scenario: ActivityScenario<CommCareActivity>,
        ...
        result.resultData.extras?.let {
            ...
            assertEqualIdentification(identificationsReturned[0], matchResults[0])
            assertThat(it.getBoolean("skipCheck")).isEqualTo(false)
        } ?: throw Exception("No bundle found")
    }

    private fun assertEqualIdentification(identification: Identification, appMatchResult: AppMatchResult) {
