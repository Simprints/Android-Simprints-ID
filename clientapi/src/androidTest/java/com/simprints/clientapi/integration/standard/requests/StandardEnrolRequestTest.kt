...
@MediumTest
class StandardEnrolRequestTest : KoinTest {
    ...
    fun anEnrolRequest_shouldLaunchAnAppEnrolRequest() {
        ActivityScenario.launch<LibSimprintsActivity>(baseIntentRequest.apply { action = idEnrolAction })
    ...
    fun aSuspiciousEnrolRequest_shouldLaunchAnAppEnrolRequest() {
        ActivityScenario.launch<LibSimprintsActivity>(suspiciousIntentRequest.apply { action = idEnrolAction })
        intended(hasAction(appEnrolAction))
    }

    @Test
    fun anInvalidEnrolRequest_shouldNotLaunchAnAppEnrolRequest() {
        ActivityScenario.launch<LibSimprintsActivity>(invalidIntentRequest.apply { action = idEnrolAction })
