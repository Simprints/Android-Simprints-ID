...
class Tokenization @Inject constructor(
    ...
    fun encrypt(value: String, keysetJson: String): String = buildDeaed(keysetJson)
        .encryptDeterministically(value.toByteArray(), null)
        .run(encodingUtils::byteArrayToBase64)

    fun decrypt(value: String, keysetJson: String): String = buildDeaed(keysetJson)
         .decryptDeterministically(encodingUtils.base64ToBytes(value), null)
        .run(::String)

    private fun buildDeaed(keysetJson: String) =
        CleartextKeysetHandle.read(JsonKeysetReader.withString(keysetJson))
