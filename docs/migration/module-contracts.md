...
These guardrails apply to **every** migration phase — architecture refactoring([viewmodel-mvi-udf.md](viewmodel-udf.md)) and 
Compose UI replacement ([compose-ui.md](compose-ui.md)). 
Nothing described here may change as a side effect of either phase.
...
- **Do not delete `graph_foo.xml`** until the Navigation 3 cutover, a separate later phase that
  requires coordinated updates to `BuildStepsUseCase` and `OrchestratorFragment` simultaneously.
...
Run this checklist before marking any module migration phase as complete:
