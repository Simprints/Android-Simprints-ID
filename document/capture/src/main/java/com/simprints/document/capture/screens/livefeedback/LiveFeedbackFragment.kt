...
import com.simprints.document.capture.databinding.FragmentLiveFeedbackBinding
...
/**
 * As the user is capturing subject's document, they are presented with this fragment, which displays
 * live information about distance and whether the document is ready to be captured or not.
 * It also displays the capture process of the document and then sends this result to
 * [com.simprints.document.capture.screens.confirmation.ConfirmationFragment]
 */
@AndroidEntryPoint
internal class LiveFeedbackFragment : Fragment(R.layout.fragment_live_feedback) {
    ...
    private fun initFragment() {
        ...
        binding.documentCaptureCamera.post {
            if (view != null) {
    ...
    private fun setUpCamera() = lifecycleScope.launch {
        ...
        preview.surfaceProvider = binding.documentCaptureCamera.surfaceProvider
        Simber.i("Camera setup finished", tag = DOCUMENT_CAPTURE)
    }
            ...
            @Suppress("WHEN_ENUM_CAN_BE_NULL_IN_JAVA") when (it) {
                ...
                LiveFeedbackFragmentViewModel.CapturingState.FINISHED -> {
                    ...
                    findNavController().navigateSafely(
                        ...
                        directions = LiveFeedbackFragmentDirections.actionDocumentLiveFeedbackFragmentToDocumentConfirmationFragment(),
                    )
                }
            }
        }
    ...
    private fun analyze(image: Bitmap) {
        try {
            ...
            Simber.e("Image analysis crashed", t, tag = DOCUMENT_CAPTURE)
        }
    }

    private fun renderCurrentDetection(documentDetection: DocumentDetection) {
        when (documentDetection.status) {
            DocumentDetection.Status.NODOCUMENT -> renderNoFace()
            DocumentDetection.Status.OFFYAW -> renderFaceNotStraight()
            DocumentDetection.Status.OFFROLL -> renderFaceNotStraight()
            DocumentDetection.Status.TOOCLOSE -> renderFaceTooClose()
            DocumentDetection.Status.TOOFAR -> renderFaceTooFar()
            DocumentDetection.Status.BAD_QUALITY -> renderBadQuality()
            DocumentDetection.Status.VALID -> renderValidFace()
            DocumentDetection.Status.VALID_CAPTURING -> renderValidCapturingFace()
        }
    }
