...
@ExcludedFromGeneratedTestCoverageReports(reason = "This class uses roc 3 class that has native functions and can't be mocked")
@Singleton
class RocV3Detector @Inject constructor() : FaceDetector {
    ...
    override fun analyze(bitmap: Bitmap): Face? {
        ...
        roc3.roc_from_rgba(byteBuffer.array(), bitmap.width.toLong(), bitmap.height.toLong(), bitmap.rowBytes.toLong(), rocColorImage)
     ...
     */
    private fun detectFace(coloredImage: roc_image, grayImage: roc_image, width: Int, height: Int): Face? {
        ...
        val face =
            if (isFaceDetected(coloredImage, detection)) {
                generateFaceTemplateFromImage(coloredImage, grayImage, detection, yaw, template, quality)
    ...
    private fun generateFaceTemplateFromImage(
        ...
        roc3.roc_embedded_landmark_face(grayImage, detection, landmarks, rightEye, leftEye, chin, null, yaw)
    ...
    private fun isFaceDetected(image: roc_image, detection: roc_detection): Boolean {

        val adaptiveMinimumSize = roc3.new_size_t()

        roc3.roc_adaptive_minimum_size(image.width, image.height, RELATIVE_MIN_SIZE, ABSOLUTE_MIN_SIZE, adaptiveMinimumSize)
    ...
    private fun roc_detection.boundingRect() =
        Rect((x - width / 2).toInt(), (y - height / 2).toInt(), (x + width / 2).toInt(), (y + height / 2).toInt())
