...
class DeleteSessionEventsIfNeededUseCaseTest {
    ...
    fun setUp() {
        ...
        deleteUseCase = DeleteSessionEventsIfNeededUseCase(configManager, clientSessionManager)
    }
    ...
    fun `deletes session events if data sync disabled`() = runTest {
        ...
        verify { clientSessionManager.deleteSessionEvents("sessionId") }
    }
    ...
    fun `does not delete session events if data sync enabled`() = runTest {
        ...
        verify(exactly = 0) { clientSessionManager.deleteSessionEvents("sessionId") }
    }
}
