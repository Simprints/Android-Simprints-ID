...
class HasDuplicateEnrolmentsUseCaseTest {
    ...
    fun `returns false if no fingerprint matches with high score`() {
        val result = useCase(
            ...
            steps = listOf(EnrolLastBiometricStepResult.FingerprintMatchResult(listOf(
    ...
    fun `returns false if no face matches with high score`() {
        val result = useCase(
            projectConfig = mockProjectConfig(),
            steps = listOf(EnrolLastBiometricStepResult.FaceMatchResult(listOf(
                matchResult(LOW_CONFIDENCE)
            )))
        )
    ...
    private fun mockProjectConfig(
        ...
        every { fingerprint?.decisionPolicy } returns highConfidence?.let { DecisionPolicy(0, 0, it) }
