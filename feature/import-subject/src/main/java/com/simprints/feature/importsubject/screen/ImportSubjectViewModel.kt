...
import com.simprints.feature.importsubject.usecase.ImportUriImageUseCase
...
internal class ImportSubjectViewModel @Inject constructor(
    private val importImage: ImportUriImageUseCase,
    ...
    fun onViewCreated(activity: Activity, projectId: String, subjectId: String, uri: String) {
        if (!importAttemted) {
            importAttemted = true
            viewModelScope.launch {
                checkLicence(activity) { importSubject(projectId, subjectId, uri) }
            }
        }
    }
    ...
    private suspend fun importSubject(projectId: String, subjectId: String, uri: String) {
        // import image bitmap
        val image = importImage(uri)
        if (image == null) {
