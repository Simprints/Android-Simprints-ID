...
internal class ShouldCreatePersonUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        val personCreationEvent = sessionEvents.filterIsInstance<PersonCreationEvent>().firstOrNull()
        // If all scheduled capture steps are complete and we don't yet have a PersonCreation event,
        // we should create one
        if (personCreationEvent == null) {
            return true
        }
        ...
        return (faceCaptureIsScheduled && !personCreationEvent.hasFaceReference()) ||
                (fingerprintCaptureIsScheduled && !personCreationEvent.hasFingerprintReference())
    }
}
