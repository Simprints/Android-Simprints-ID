...
internal class CreatePersonEventUseCaseTest {
    ...
    @Test
    fun `Gets fingerprint from previous PersonCreationEvent (when present) if missing in current callout captures`() = runTest {
        coEvery { eventRepository.getEventsInCurrentSession() } returns listOf(
    ...
    @Test
    fun `Gets face from previous PersonCreationEvent (when present) if missing in current callout captures`() = runTest {
        coEvery { eventRepository.getEventsInCurrentSession() } returns listOf(
            mockk<PersonCreationEvent> {
    ...
    @Test
    fun `Uses face from latest PersonCreationEvent (if more than one) if missing in current callout captures`() = runTest {
        coEvery { eventRepository.getEventsInCurrentSession() } returns listOf(
            mockk<PersonCreationEvent> {
                every { payload.faceCaptureIds } returns listOf(FACE_CAPTURE_ID)
                every { payload.faceReferenceId } returns FACE_REFERENCE_ID
    ...
    private fun createFaceCaptureResultItem() = FaceCaptureResult.Item(
