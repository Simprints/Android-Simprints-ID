...
class OtaFragment : FingerprintFragment() {
    ...
    private fun setTextInLayout() {
        binding.apply {
            otaTitleTextView.text = getString(IDR.string.ota_title)
            otaInstructionsTextView.text = getString(IDR.string.ota_instructions)
            startUpdateButton.text = getString(IDR.string.start_update)
        }
    }
    ...
    private fun listenForRecoveryEvent() {
        viewModel.otaRecovery.fragmentObserveEventWith {
            findNavController().navigate(
                OtaFragmentDirections.actionOtaFragmentToOtaRecoveryFragment(it)
            )
        }
    }
    ...
    private fun listenForFailedEvent() {
        viewModel.otaFailed.fragmentObserveEventWith {
            findNavController().navigate(
                OtaFragmentDirections.actionOtaFragmentToOtaFailedFragment(it)
            )
        }
    }
