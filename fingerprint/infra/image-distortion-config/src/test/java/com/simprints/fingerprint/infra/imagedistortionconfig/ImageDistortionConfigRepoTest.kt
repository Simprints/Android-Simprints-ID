...
class ImageDistortionConfigRepoTest {
    ...
    fun `saveConfig should save to localRepo and upload pending configs`() = runBlocking {
        ...
        val serialNumber = "serial456"
        val configFile = byteArrayOf(1, 2, 3, 4)

        coJustRun { localRepo.saveConfig(scannerId, serialNumber, configFile) }
        ...
        // Then
        coVerify(exactly = 1) { localRepo.saveConfig(scannerId, serialNumber, configFile) }
    ...
    fun `uploadPendingConfigs should upload all pending configs and mark as uploaded`() = runTest {
        ...
        coVerifySequence {
            ...
            remoteRepo.uploadConfig(pendingConfigs[0].serialNumber, pendingConfigs[0].configFile)
            localRepo.markAsUploaded(1)
            remoteRepo.uploadConfig(pendingConfigs[1].serialNumber, pendingConfigs[1].configFile)
    ...
    fun `uploadPendingConfigs should return false if upload fails`() = runTest {
        ...
        coEvery { localRepo.getPendingUploads() } returns pendingConfigs
        coEvery { remoteRepo.uploadConfig(pendingConfigs[0].serialNumber, pendingConfigs[0].configFile) } returns true
        coEvery { remoteRepo.uploadConfig(pendingConfigs[1].serialNumber, pendingConfigs[1].configFile) } returns false
        coJustRun { localRepo.markAsUploaded(any()) }
        ...
        // Then
        coVerify(exactly = 1) { remoteRepo.uploadConfig(pendingConfigs[0].serialNumber, pendingConfigs[0].configFile) }
        coVerify(exactly = 1) { localRepo.markAsUploaded(1) }
        coVerify(exactly = 1) { remoteRepo.uploadConfig(pendingConfigs[1].serialNumber, pendingConfigs[1].configFile) }
