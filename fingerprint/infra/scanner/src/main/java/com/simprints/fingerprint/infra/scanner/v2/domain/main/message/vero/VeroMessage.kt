...
sealed class VeroMessage(
    val veroMessageType: VeroMessageType,
) : MainMessage {
    override fun getBytes(): ByteArray = VeroMessageProtocol.buildMessageBytes(veroMessageType, getDataBytes())

    open fun getDataBytes() = byteArrayOf()
}

abstract class VeroCommand(
    veroMessageType: VeroMessageType,
) : VeroMessage(veroMessageType),
    OutgoingMainMessage

abstract class VeroResponse(
    veroMessageType: VeroMessageType,
) : VeroMessage(veroMessageType),
    IncomingMainMessage

abstract class VeroEvent(
    veroMessageType: VeroMessageType,
) : VeroMessage(veroMessageType),
    IncomingMainMessage
