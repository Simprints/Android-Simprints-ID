...
class GetPowerLedStateResponse(
    val ledState: LedState,
) : VeroResponse(VeroMessageType.GET_POWER_LED_STATE) {
    override fun getDataBytes(): ByteArray = ledState.getBytes()

    companion object {
        fun fromBytes(data: ByteArray) = GetPowerLedStateResponse(LedState.fromBytes(data))
    }
}
