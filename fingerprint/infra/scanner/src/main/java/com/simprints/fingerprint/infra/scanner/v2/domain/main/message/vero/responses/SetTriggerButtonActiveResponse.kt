...
class SetTriggerButtonActiveResponse(
    val operationResultCode: OperationResultCode,
) : VeroResponse(VeroMessageType.SET_TRIGGER_BUTTON_ACTIVE) {
    override fun getDataBytes(): ByteArray = byteArrayOf(operationResultCode.byte)

    companion object {
        fun fromBytes(data: ByteArray) = SetTriggerButtonActiveResponse(OperationResultCode.fromBytes(data))
    }
}
