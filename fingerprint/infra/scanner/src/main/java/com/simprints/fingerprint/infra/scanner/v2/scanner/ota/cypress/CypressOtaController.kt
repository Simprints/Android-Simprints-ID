...
class CypressOtaController(private val crc32Calculator: Crc32Calculator) {
    ...
    suspend fun program(
        cypressOtaMessageChannel: CypressOtaMessageChannel, firmwareBinFile: ByteArray
    ): Flow<Float> {
        ...
        return chunks.asFlow().map { (chunk, progress) ->
            sendImageChunk(cypressOtaMessageChannel, chunk)
            progress
        }.onCompletion {
            //verify OTA is OK
            sendVerifyImageCommand(
                cypressOtaMessageChannel, crc32Calculator.calculateCrc32(firmwareBinFile)
            )
        }
    }
    ...
    ) = cypressOtaMessageChannel.sendCypressOtaModeCommandAndReceiveResponse<CypressOtaResponse>(
    ...
    private fun CypressOtaResponse.verifyResponseIs(
        type: CypressOtaResponseType, commandName: String
    ) {
        if (this.type != type) {
