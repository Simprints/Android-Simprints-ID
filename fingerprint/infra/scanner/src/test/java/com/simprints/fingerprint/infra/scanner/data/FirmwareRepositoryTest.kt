...
class FirmwareRepositoryTest {
    ...
    @Test
    fun `updateStoredFirmwareFilesWithLatest downloads the latest file versions when available`() = runTest {
    ...
    @Test
    fun `updateStoredFirmwareFilesWithLatest downloads no files when no versions available`() = runTest {
        coEvery {
            firmwareLocalDataSourceMock.getAvailableScannerFirmwareVersions()
        } returns SCANNER_VERSIONS_LOW
        every {
    ...
    @Test
    fun `updateStoredFirmwareFilesWithLatest downloads other files when only one versions available`() = runTest {
        coEvery { firmwareRemoteDataSourceMock.downloadFirmware(any()) } returns CYPRESS_BIN
        coEvery {
            firmwareLocalDataSourceMock.getAvailableScannerFirmwareVersions()
    ...
    @Test
    fun `cleanUpOldFirmwareFiles should remove nothing if all firmware versions are up to date`() = runTest {
        // Given
        coEvery {
            firmwareLocalDataSourceMock.getAvailableScannerFirmwareVersions()
