...
class ScannerHardwareRevisionsTest {
    ...
    fun `test getDownloadableFirmwares with empty local ScannerFirmwareVersions `() {
        //Given
        val scannerHardwareRevisions = com.simprints.fingerprint.infra.scanner.data.FirmwareTestData.RESPONSE_HARDWARE_REVISIONS_MAP
        val local = emptyMap<DownloadableFirmwareVersion.Chip, Set<String>>()
        // When
        val result =
            scannerHardwareRevisions.getAvailableVersionsForDownload(
                com.simprints.fingerprint.infra.scanner.data.FirmwareTestData.HARDWARE_VERSION,
                local
            )
        // Then
