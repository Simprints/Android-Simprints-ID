...
class SimulatedVeroResponseHelper(
    ...
    override fun createResponseToCommand(command: VeroCommand): VeroResponse {
        val response = when (command) {
            ...
            is GetUn20OnCommand -> GetUn20OnResponse(
                if (simulatedScannerV2.scannerState.isUn20On) DigitalValue.TRUE else DigitalValue.FALSE,
            )
            ...
            is GetBatteryPercentChargeCommand -> GetBatteryPercentChargeResponse(
                BatteryPercentCharge(simulatedScannerV2.scannerState.batteryPercentCharge.toByte()),
            )
            is GetBatteryVoltageCommand -> GetBatteryVoltageResponse(
                BatteryVoltage(simulatedScannerV2.scannerState.batteryVoltageMilliVolts.toShort()),
            )
            is GetBatteryCurrentCommand -> GetBatteryCurrentResponse(
                BatteryCurrent(simulatedScannerV2.scannerState.batteryCurrentMilliAmps.toShort()),
            )
            is GetBatteryTemperatureCommand -> GetBatteryTemperatureResponse(
                BatteryTemperature(simulatedScannerV2.scannerState.batteryTemperatureDeciKelvin.toShort()),
            )
