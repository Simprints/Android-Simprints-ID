...
class SerialNumberConverter {
     ...
     */
    fun convertMacAddressToSerialNumber(address: String): String =
        SERIAL_PREFIX + address.replace(":", "").substring(7..11)
            .toInt(16).rem(SERIAL_NUMBER_LIMIT).toString(10).padStart(6, '0')

    /**
     * @param serialNumber the serial number in "SPXXXXXX" format
     * @return the MAC address in "F0:AC:D7:CX:XX:XX" format
     */
    fun convertSerialNumberToMacAddress(serialNumber: String): String =
        serialNumber.removePrefix(SERIAL_PREFIX).toInt().let { serialInt ->
    ...
    companion object {
        ...
        val VERO_1_WITH_MAC_ADDRESS_ABOVE_SERIAL_NUMBER_LIMIT = listOf(
            "SP018257", "SP009113", "SP047961", "SP023967", "SP000652"
        )
    }
}
