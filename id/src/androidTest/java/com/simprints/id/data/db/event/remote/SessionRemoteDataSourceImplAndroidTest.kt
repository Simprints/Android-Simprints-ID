...
class SessionRemoteDataSourceImplAndroidTest {
    ...
    private suspend fun executeUpload(sessions: MutableList<SessionCaptureEvent>) {
    ...
    private fun SessionCaptureEvent.addAuthorizationEvent() {
    ...
    private fun SessionCaptureEvent.addCandidateReadEvent() {
    ...
    private fun SessionCaptureEvent.addConnectivitySnapshotEvent() {
    ...
    private fun SessionCaptureEvent.addConsentEvent() {
    ...
    private fun SessionCaptureEvent.addEnrolmentEvent() {
        addEvent(EnrolmentEvent(0, RANDOM_GUID))
    }

    private fun SessionCaptureEvent.addFingerprintCaptureEvent() {
    ...
    private fun SessionCaptureEvent.addGuidSelectionEvent() {
        addEvent(GuidSelectionEvent(0, RANDOM_GUID))
    }

    private fun SessionCaptureEvent.addIntentParsingEvent() {
        IntentParsingEvent.IntegrationInfo.values().forEach {
            addEvent(IntentParsingEvent(0, it))
        }
    }

    private fun SessionCaptureEvent.addInvalidIntentEvent() {
        addEvent(InvalidIntentEvent(0, "some_action", emptyMap()))
    }

    private fun SessionCaptureEvent.addOneToManyMatchEvent() {
    ...
    private fun SessionCaptureEvent.addOneToOneMatchEvent() {
        addEvent(OneToOneMatchEvent(0, 0, RANDOM_GUID, MatchEntry(RANDOM_GUID, 0F)))
    }

    private fun SessionCaptureEvent.addPersonCreationEvent() {
        addEvent(PersonCreationEvent(0, listOf(RANDOM_GUID, RANDOM_GUID)))
    }

    private fun SessionCaptureEvent.addRefusalEvent() {
        RefusalEvent.Answer.values().forEach {
            addEvent(RefusalEvent(0, 0, it, "other_text"))
        }
    }

    private fun SessionCaptureEvent.addScannerConnectionEvent() {
    ...
    private fun SessionCaptureEvent.addVero2InfoSnapshotEvents() {
    ...
    private fun SessionCaptureEvent.addScannerFirmwareUpdateEvent() {
        addEvent(ScannerFirmwareUpdateEvent(0, 0, "stm",
            "targetApp", "failureReason"))
    }

    private fun SessionCaptureEvent.addSuspiciousIntentEvent() {
        addEvent(SuspiciousIntentEvent(0, mapOf("some_extra_key" to "value")))
    }

    private fun SessionCaptureEvent.addCompletionCheckEvent() {
        addEvent(CompletionCheckEvent(0, true))
    }

    private fun SessionCaptureEvent.addCallbackEvent() {
    ...
    private fun SessionCaptureEvent.addCalloutEvent() {
