...
class PersonCreationEventHelperImpl(
    ...
    override suspend fun addPersonCreationEventIfNeeded(steps: List<Result>) {
        ...
        val personCreationEventInSession =
            eventRepository.loadEvents(currentSession.id).filterIsInstance<PersonCreationEvent>().toList()
    ...
    private suspend fun addPersonCreationEvent(
        ...
        val fingerprintCaptureEvents =
            eventRepository.loadEvents(currentCaptureSessionEvent.id).filterIsInstance<FingerprintCaptureEvent>()
                .toList()
        val faceCaptureEvents =
            eventRepository.loadEvents(currentCaptureSessionEvent.id).filterIsInstance<FaceCaptureEvent>().toList()
