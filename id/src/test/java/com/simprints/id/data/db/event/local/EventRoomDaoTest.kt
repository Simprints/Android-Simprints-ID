...
class EventRoomDaoTest {
    ...
    fun loadById() {
        runBlocking {
            ...
            verifyEvents(listOf(event), eventDao.load(id = SOME_GUID1))
        }
    }
    ...
    fun loadByProjectId() {
        runBlocking {
            val wrongEvent = event.copy(id = randomUUID(), labels = EventLabels(projectId = SOME_GUID1))
            addIntoDb(event, wrongEvent)
    ...
    fun loadBySubjectId() {
        runBlocking {
            val wrongEvent = event.copy(id = randomUUID(), labels = EventLabels(subjectId = SOME_GUID1))
            addIntoDb(event, wrongEvent)
    ...
    fun loadByAttendantId() {
        runBlocking {
            val wrongEvent = event.copy(id = randomUUID(), labels = EventLabels(attendantId = SOME_GUID1))
            addIntoDb(event, wrongEvent)
    ...
    fun loadBySessionId() {
        runBlocking {
            val wrongEvent = event.copy(id = randomUUID(), labels = EventLabels(sessionId = SOME_GUID2))
            addIntoDb(event, wrongEvent)
            verifyEvents(listOf(event), eventDao.load(sessionId = SOME_GUID1))
        }
    }
    ...
    fun loadByDeviceId() {
        runBlocking {
            val wrongEvent = event.copy(id = randomUUID(), labels = EventLabels(deviceId = SOME_GUID2))
            addIntoDb(event, wrongEvent)
            verifyEvents(listOf(event), eventDao.load(deviceId = SOME_GUID1))
        }
    }
    ...
    fun deletionBySessionId() {
        runBlocking {
            val eventSameSession = event.copy(id = randomUUID(), labels = EventLabels(sessionId = SOME_GUID1))
            val eventDifferentSession = event.copy(id = randomUUID(), labels = EventLabels(sessionId = SOME_GUID2))
            addIntoDb(event, eventSameSession, eventDifferentSession)
            db.eventDao.delete(sessionId = SOME_GUID1)
