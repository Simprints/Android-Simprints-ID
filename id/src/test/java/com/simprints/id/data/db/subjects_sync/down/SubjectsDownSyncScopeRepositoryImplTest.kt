...
class SubjectsDownSyncScopeRepositoryImplTest {
    ...
    private val projectSyncOp = SubjectsDownSyncOperation(DEFAULT_PROJECT_ID, null, null, DEFAULT_MODES, null)
    private val userSyncOp = SubjectsDownSyncOperation(DEFAULT_PROJECT_ID, DEFAULT_USER_ID, null, DEFAULT_MODES, null)
    private val moduleSyncOp = SubjectsDownSyncOperation(DEFAULT_PROJECT_ID, null, DEFAULT_MODULE_ID, DEFAULT_MODES, null)
    private val lastDownSyncResult = SubjectsDownSyncOperationResult(COMPLETE, LAST_PATIENT_ID, LAST_PATIENT_UPDATED_AT)

    @MockK lateinit var loginInfoManager: LoginInfoManager
    @MockK lateinit var preferencesManager: PreferencesManager
    @MockK lateinit var downSyncOperationOperationDao: SubjectsDownSyncOperationLocalDataSource
    @MockK lateinit var subjectsDownSyncOperationFactory: SubjectsDownSyncOperationFactory
    ...
    fun setUp() {
        ...
        subjectsDownSyncScopeRepository = spyk(SubjectsDownSyncScopeRepositoryImpl(loginInfoManager, preferencesManager, downSyncOperationOperationDao, subjectsDownSyncOperationFactory))
    ...
    fun givenProjectSyncGroup_getDownSyncOperations_shouldReturnSyncOps() = runBlockingTest {
        every { subjectsDownSyncOperationFactory.buildProjectSyncOperation(any(), any(), any()) } returns projectSyncOp
        ...
        with(projectSyncScope) {
            verify { subjectsDownSyncOperationFactory.buildProjectSyncOperation(projectId, modes, null) }
    ...
    fun givenUserSyncGroup_getDownSyncOperations_shouldReturnSyncOps() = runBlockingTest {
        every { subjectsDownSyncOperationFactory.buildUserSyncOperation(any(), any(), any(), any()) } returns userSyncOp
        ...
        with(userSyncScope) {
            verify { subjectsDownSyncOperationFactory.buildUserSyncOperation(projectId, userId, modes, null) }
    ...
    fun givenModuleSyncGroup_getDownSyncOperations_shouldReturnSyncOps() = runBlockingTest {
        every { subjectsDownSyncOperationFactory.buildModuleSyncOperation(any(), any(), any(), any()) } returns moduleSyncOp
        ...
        with(moduleSyncScope) {
            verify { subjectsDownSyncOperationFactory.buildModuleSyncOperation(projectId, modules[0], modes, null) }
            verify { subjectsDownSyncOperationFactory.buildModuleSyncOperation(projectId, modules[1], modes, null) }
    ...
    private fun SubjectsDownSyncOperation.assertModuleSyncOpIsRefreshed() {
        assertThat(lastResult).isEqualTo(lastDownSyncResult)
