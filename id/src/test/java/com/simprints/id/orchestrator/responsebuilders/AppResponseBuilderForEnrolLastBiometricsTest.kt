...
class AppResponseBuilderForEnrolLastBiometricsTest {
    ...
    fun givenAnAppBuilder_withAValidLastBiometricStepResult_shouldBuildAppResponse() {
        runBlockingTest {
            ...
            val step = Step(
                ...
                request = EnrolLastBiometricsRequest(DEFAULT_PROJECT_ID, DEFAULT_USER_ID, DEFAULT_MODULE_ID, emptyList(), SOME_GUID1),
                status = Step.Status.NOT_STARTED
            )
            step.setResult(EnrolLastBiometricsResponse(SOME_GUID1))
            val response = lastEnBuilderForEnrolLastBiometrics.buildAppResponse(mockk(), mockk(), listOf(step), SOME_GUID1) as AppEnrolResponse

            assertThat(response.guid).isEqualTo(SOME_GUID1)
        }
    }
    ...
    fun givenAnAppBuilder_withAnInvalidLastBiometricStepResult_shouldBuildAppResponse() {
        runBlockingTest {
            ...
            val response = lastEnBuilderForEnrolLastBiometrics.buildAppResponse(mockk(), mockk(), listOf(step), SOME_GUID1)
