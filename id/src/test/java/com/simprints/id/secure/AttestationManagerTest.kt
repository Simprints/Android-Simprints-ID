...
class AttestationManagerImplTest : AutoCloseKoinTest() {
    ...
    @MockK
    lateinit var safetyNetClientMock: SafetyNetClient
    @MockK
    lateinit var safetyNetAttestationResponseMock: SafetyNetApi.AttestationResponse
    ...
    fun attestationWithErrors_shouldThrowWithInvalidClaims() {
        ...
        assertThat(
            assertThrows<SafetyNetException> {
                attestationManagerSpy.requestAttestation(
                    safetyNetClientMock,
                    nonce
                )
            }.reason
    ...
    @Test
    fun attestationWithNull_shouldThrowWithInvalidClaims() {
        mockSafetyNetResponseResult(ERROR_JWS_RESULT)
        mockAttestationManagerToReturnSafetyNetResponse(safetyNetAttestationResponseMock)

        every { safetyNetAttestationResponseMock.jwsResult } returns null
    ...
    fun attestationFailedToRetrieve_shouldThrowWithUnavailable() {
        every { safetyNetClientMock.attest(any(), any()) } throws
            Throwable(
                "Google SafetyNet CANCELED result",
    ...
    private fun mockAttestationManagerToReturnSafetyNetResponse(safetyNetAttestationResponseMock: SafetyNetApi.AttestationResponse) {
        every {
            attestationManagerSpy.getSafetyNetAttestationResponse(
                safetyNetClientMock,
                nonce
            )
        } returns safetyNetAttestationResponseMock
    }
