...
class AttestationManagerTest {
    ...
    fun validAttestation_shouldSucceed() {
        ...
        val safetyNetClientMock: SafetyNetClient = mock()
        val safetyNetAttestationResponseMock: SafetyNetApi.AttestationResponse = mock()
        val nonce = Nonce("nonce")

        whenever { safetyNetAttestationResponseMock.jwsResult } thenReturn VALID_JWS_RESULT

        val attestationManager = spy(AttestationManager()).apply {
            doReturn(safetyNetAttestationResponseMock).`when`(this).getSafetyNetAttestationResponse(safetyNetClientMock, nonce)
        }

        val testObserver = attestationManager.requestAttestation(safetyNetClientMock, nonce).test()
    ...
    fun attestationWithErrors_shouldThrowWithInvalidClaims() {
        ...
        whenever { safetyNetAttestationResponseMock.jwsResult } thenReturn ERROR_JWS_RESULT
        ...
        testObserver.assertError(SafetyNetException::class.java)
        Truth.assertThat(testObserver.events[1][0] is SafetyNetException).isTrue()
        Truth.assertThat((testObserver.events[1][0] as SafetyNetException).reason).isEqualTo(SafetyNetExceptionReason.INVALID_CLAIMS)
    }
    ...
    fun attestationFailedToRetrieve_shouldThrowWithUnavailable() {
        ...
        val attestationManager = spy(AttestationManager())
        ...
        testObserver.assertError(SafetyNetException::class.java)
        Truth.assertThat(testObserver.events[1][0] is SafetyNetException).isTrue()
        Truth.assertThat((testObserver.events[1][0] as SafetyNetException).reason).isEqualTo(SafetyNetExceptionReason.SERVICE_UNAVAILABLE)
    }
