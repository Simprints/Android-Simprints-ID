...
class SimApiMock(private val delegate: BehaviorDelegate<SyncApiInterface>) : SyncApiInterface {
    ...
    override fun upSync(patientsJson: HashMap<String, ArrayList<fb_Person>>, apiKey: String): Completable {
        return delegate.returning(buildSuccessResponseWith("")).upSync(patientsJson)
    }

    override fun downSync(date: Long, syncParams: Map<String, String>, batchSize: Int, apiKey: String): Single<ResponseBody> {
        return delegate.returning(buildSuccessResponseWith("")).downSync(date, syncParams)
    }

    override fun getPatient(patientId: String, projectId: String, apiKey: String): Single<ArrayList<fb_Person>> {
        return delegate.returning(buildSuccessResponseWith("")).getPatient(patientId, projectId)
    }

    override fun patientsCount(syncParams: Map<String, String>, apiKey: String): Single<PatientsCount> {
