...
class ProjectConfigFaceBioSdkMigrationTest {
    ...
    fun `should migrate if face has no rankone config`() = runTest {
        val currentData = ProtoProjectConfiguration.newBuilder().setFace(
            ProtoFaceConfiguration.newBuilder()
                .addAllowedSdks(ProtoFaceConfiguration.ProtoBioSdk.RANK_ONE).build()
        ).build()
    ...
    fun `should migrate if face allowedSdks is empty `() = runTest {
        val currentData = ProtoProjectConfiguration.newBuilder().setFace(
            ProtoFaceConfiguration.newBuilder().setRankOne(
                ProtoFaceConfiguration.ProtoFaceSdkConfiguration.newBuilder()
                    .setNbOfImagesToCapture(5).setQualityThreshold(1)
                    .setImageSavingStrategy(ProtoFaceConfiguration.ImageSavingStrategy.NEVER)
                    .setDecisionPolicy(protoDecisionPolicy).build()
            ).build()
        ).build()
    ...
    fun `should create rankone with value of the old face configuration`() = runTest {
        ...
        val currentData = ProtoProjectConfiguration.newBuilder().setFace(
            ProtoFaceConfiguration.newBuilder().setNbOfImagesToCapture(nbOfImagesToCapture)
