...
class ProjectConfigSharedPrefsMigrationTest {
    ...
    @Test
    fun `shouldMigrate should return true only if the current data is empty and the shared pref is not empty`() = runTest {
    ...
    @Test
    fun `shouldMigrate should return false if the current data is empty`() = runTest {
        every { preferences.getString(PROJECT_SETTINGS_JSON_STRING_KEY, "") } returns ""
        ...
        val expectedProto = ProtoProjectConfiguration
            ...
            .setFingerprint(
                PROTO_FINGERPRINT_CONFIGURATION.toBuilder().setSecugenSimMatcher(
                    PROTO_FINGERPRINT_CONFIGURATION.secugenSimMatcher
                        .toBuilder()
                        .clearVero2()
                        .build(),
                ),
    ...
    fun `migrate should throw an exception if there is an issue that is not related to serialization`() = runTest {
        ...
        mockkObject(SimJson)
        every { SimJson.decodeFromString<OldProjectConfig>(any()) } throws exception
    ...
    private fun concatMapsAsString(vararg maps: Map<String, Any>): String {
        ...
        return SimJson.encodeToString(m)
    }
    ...
    companion object {
        ...
        private val JSON_GENERAL_CONFIGURATION =
            SimJson.decodeFromString<Map<String, String>>(
        ...
        private val PROTO_GENERAL_CONFIGURATION = ProtoGeneralConfiguration
            .newBuilder()
            .addAllModalities(
                listOf(
        ...
        private val PROTO_CONSENT_CONFIGURATION = ProtoConsentConfiguration
            .newBuilder()
        ...
        private val PROTO_IDENTIFICATION_CONFIGURATION =
            ProtoIdentificationConfiguration
                .newBuilder()
                .setMaxNbOfReturnedCandidates(4)
                .setPoolType(ProtoIdentificationConfiguration.PoolType.MODULE)
                .build()

        private val JSON_FACE_CONFIGURATION = SimJson.decodeFromString<Map<String, String>>(
        ...
        private val JSON_FACE_CONFIGURATION_WITH_UNEXPECTED_FIELD =
            SimJson.decodeFromString<Map<String, JsonElement>>(
                """
                {
        ...
        private val PROTO_VERO_2_CONFIGURATION = ProtoVero2Configuration
            .newBuilder()
        ...
        private val PROTO_FINGERPRINT_CONFIGURATION = ProtoFingerprintConfiguration
            .newBuilder()
