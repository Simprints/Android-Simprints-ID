...
import com.simprints.infra.config.local.models.ProtoFingerprintConfiguration.VeroGeneration.*
...
import com.simprints.infra.config.local.models.ProtoVero2Configuration.CaptureStrategy.SECUGEN_ISO_500_DPI
...
class ProjectConfigQualityThresholdMigrationTest {
    ...
    @Test
    fun `shouldMigrate should return false if the current data doesn't have a fingerprint field`() =
        runTest {
    ...
    @Test
    fun `shouldMigrate should return false if the current data has an empty quality threshold for fingerprint`() =
        runTest {
            val proto = ProtoProjectConfiguration.newBuilder()
                .setFingerprint(
    ...
    @Test
    fun `shouldMigrate should return true if the current data has a non empty quality threshold for fingerprint`() =
        runTest {
            val proto = ProtoProjectConfiguration.newBuilder()
                .setFingerprint(
    ...
    @Test
    fun `migrate should migrate both vero1 and vero2`() =
        runTest {
            val proto = ProtoProjectConfiguration.newBuilder()
                .setFingerprint(
                    ProtoFingerprintConfiguration.newBuilder()
                        ...
                        .setVero2(
                            ProtoVero2Configuration.newBuilder()
                                .setCaptureStrategy(SECUGEN_ISO_500_DPI).build()
                        )
                        .build()
                )
                .build()
