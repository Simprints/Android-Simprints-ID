...
internal class RealmToRoomMigrationWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result = withContext(dispatcher) {
        ...
        crashlyticsLog("MigrationWorker started.")
        try {
            ...
            if (realmToRoomMigrationFlagsStore.isDownSyncInProgress()) {
                ...
                crashlyticsLog("Down sync is in progress. pausing migration.")
            ...
            if (recordsCount > 0) {
                ...
                // log the time taken for migration to help with debugging
                crashlyticsLog("migrated $recordsCount in ${time.inWholeMilliseconds}ms")
            } else {
                crashlyticsLog("No records to migrate. Skipping migration.")
            }
            realmToRoomMigrationFlagsStore.updateStatus(MigrationStatus.COMPLETED)
            realmToRoomMigrationFlagsStore.resetRetryCount()
            return@withContext success()
        } catch (e: Exception) {
            Simber.e("Migration failed: ${e.message}", e)
    ...
    private suspend fun processRecords() {
        ...
        var index = 0
        realmDataSource
            .loadAllSubjectsInBatches(BATCH_SIZE, scope)
            .consumeEach { realmSubjectsBatch ->
                crashlyticsLog("Processing batch ${++index} ...")
                try {
                    ...
                    Simber.e("Error processing batch ${index + 1}: ${e.message}", e)
                    throw e
                }
            }
    }
