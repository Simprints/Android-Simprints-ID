...
internal fun RealmSubject.toDomain(): Subject {
    ...
    return Subject(
        ...
        references = fingerprintSamples.toDomain() + faceSamples.toDomain(),
...
internal fun Subject.toRealmDb(): RealmSubject = RealmSubject().also { subject ->
    ...
    subject.fingerprintSamples = references
        .filter { it.modality == Modality.FINGERPRINT }
        .flatMap(BiometricReference::toRealmFingerprintDb)
        .toRealmList()
    subject.faceSamples = references
        .filter { it.modality == Modality.FACE }
        .flatMap(BiometricReference::toRealmFaceDb)
        .toRealmList()
}
