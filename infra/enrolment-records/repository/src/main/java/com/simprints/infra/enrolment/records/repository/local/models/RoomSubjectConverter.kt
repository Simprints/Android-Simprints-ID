...
internal fun SubjectBiometrics.toDomain() = Subject(
    ...
    fingerprintSamples = biometricTemplates.filter { it.modality == Modality.FINGERPRINT.ordinal }.map { it.toFingerprintSample() },
    faceSamples = biometricTemplates.filter { it.modality == Modality.FACE.ordinal }.map { it.toFaceSample() },
)
