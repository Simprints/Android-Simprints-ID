    ...
    fun performSubjectUpdateAction() = runTest {
        ...
        verify {
            ...
            mutableRealm.copyToRealm(
                withArg<DbSubject> {
                    // one old + one new
                    assert(it.faceSamples.size == 2)
                    assert(it.fingerprintSamples.size == 2)
                    assert(it.faceSamples.none { sample -> sample.referenceId == faceReferenceId })
                    assert(it.fingerprintSamples.none { sample -> sample.referenceId == fingerReferenceId })
                },
                any(),
            )
        }
