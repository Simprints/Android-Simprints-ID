...
class RoomEnrollmentRecordLocalDataSourceTest {
    ...
    private val faceSample3Project2 = FaceSample(
    ...
    private val fingerprintSample3Project2 = FingerprintSample(
    ...
    private val subject4P2WithBoth = Subject(
        ...
        fingerprintSamples = listOf(fingerprintSample3Project2),
        createdAt = date,
        updatedAt = date,
    )
    ...
    private suspend fun setupInitialData() {
        dataSource.performActions(
            initialData,
            project, // Use the mocked project instance
        )
    }
    ...
    @Test
    fun `load - by faceSampleFormat - should return subjects with matching format`() = runTest {
        // Given
        setupInitialData()

        // When
        val loadedP1Roc1 =
            dataSource.load(SubjectQuery(projectId = PROJECT_1_ID, faceSampleFormat = ROC_1_FORMAT))
        val loadedP1Roc3 =
            dataSource.load(SubjectQuery(projectId = PROJECT_1_ID, faceSampleFormat = ROC_3_FORMAT))
        val loadedP2Roc1 =
            dataSource.load(SubjectQuery(projectId = PROJECT_2_ID, faceSampleFormat = ROC_1_FORMAT))
        val loadedAllRoc1 = dataSource.load(SubjectQuery(faceSampleFormat = ROC_1_FORMAT))
        ...
        assertThat(loadedP2Roc1).hasSize(1)
        assertThat(loadedP2Roc1[0].subjectId).isEqualTo(subject5P2WithFace.subjectId)

        assertThat(loadedAllRoc1).hasSize(2)
        assertThat(loadedAllRoc1.map { it.subjectId }).containsExactly(
            subject1P1WithFace.subjectId,
            subject5P2WithFace.subjectId,
        )
    }

    @Test
    fun `load - by fingerprintSampleFormat - should return subjects with matching format`() = runTest {
        // Given
        setupInitialData()

        // When
        val loadedP1Nec = dataSource.load(
            SubjectQuery(
                projectId = PROJECT_1_ID,
                fingerprintSampleFormat = NEC_FORMAT,
            ),
        )
        val loadedP1Iso = dataSource.load(
            SubjectQuery(
                projectId = PROJECT_1_ID,
                fingerprintSampleFormat = ISO_FORMAT,
            ),
        )
        val loadedP2Nec = dataSource.load(
            SubjectQuery(
                projectId = PROJECT_2_ID,
                fingerprintSampleFormat = NEC_FORMAT,
            ),
        )
        val loadedAllNec = dataSource.load(SubjectQuery(fingerprintSampleFormat = NEC_FORMAT))
        ...
        assertThat(loadedP2Nec).hasSize(1)
        assertThat(loadedP2Nec[0].subjectId).isEqualTo(subject6P2WithFinger.subjectId)

        assertThat(loadedAllNec).hasSize(2)
        assertThat(loadedAllNec.map { it.subjectId }).containsExactly(
            subject2P1WithFinger.subjectId,
    ...
    fun `loadFingerprintIdentities - should load identities matching format for the specified project`() = runTest {
        ...
        val loadedP1Nec = dataSource.loadFingerprintIdentities(
            query = SubjectQuery(
                projectId = PROJECT_1_ID,
                fingerprintSampleFormat = NEC_FORMAT,
            ),
        ...
        val loadedP1Iso = dataSource.loadFingerprintIdentities(
            query = SubjectQuery(
                projectId = PROJECT_1_ID,
                fingerprintSampleFormat = ISO_FORMAT,
            ),
        ...
        val loadedP2Nec = dataSource.loadFingerprintIdentities(
            query = SubjectQuery(
                projectId = PROJECT_2_ID,
                fingerprintSampleFormat = NEC_FORMAT,
            ),
    ...
    fun `loadFingerprintIdentities - should respect the range parameter for specific format`() = runTest {
        ...
        // Then
        assertThat(loadedFirst).hasSize(1)
        assertThat(loadedFirst[0].subjectId).isEqualTo(subject2P1WithFinger.subjectId)
        assertThat(loadedSecond).hasSize(1)
        assertThat(loadedSecond[0].subjectId).isEqualTo(subject5P1WithNec.subjectId)
    ...
    fun `loadFingerprintIdentities - with query format matching nothing - should return empty list`() = runTest {
        ...
        // When
        val loadedIdentities = dataSource.loadFingerprintIdentities(
            query = SubjectQuery(
                projectId = PROJECT_1_ID,
    ...
    fun `loadFaceIdentities - should respect the range parameter for specific format`() = runTest {
        ...
        // Then
        assertThat(loadedFirst).hasSize(1)
        assertThat(loadedFirst[0].subjectId).isEqualTo(subject1P1WithFace.subjectId)
        assertThat(loadedSecond).hasSize(1)
        assertThat(loadedSecond[0].subjectId).isEqualTo(subject5P1WithRoc1.subjectId)
        assertThat(loadedFirstTwo).hasSize(2)
        assertThat(loadedFirstTwo.map { it.subjectId })
    ...
    fun `loadFaceIdentities - with query format matching nothing - should return empty list`() = runTest {
        ...
        // When
        val loadedIdentities = dataSource.loadFaceIdentities(
            query = SubjectQuery(projectId = PROJECT_1_ID, faceSampleFormat = UNUSED_FORMAT),
            range = 0..10,
            project = project,
            onCandidateLoaded = mockCallback,
        )
    ...
    fun `loadFaceIdentities - by attendantId and moduleId - should return correct identities`() = runTest {
        ...
        // When
        val loadedP1A1M1Roc1 = dataSource.loadFaceIdentities(
            queryP1A1M1Roc1,
            0..10,
            project = project,
            onCandidateLoaded = mockCallback,
        )
        val loadedP1A1M2Roc3 = dataSource.loadFaceIdentities(
            queryP1A1M2Roc3,
            0..10,
            project = project,
            onCandidateLoaded = mockCallback,
    ...
    fun `loadFingerprintIdentities - by attendantId and moduleId - should return correct identities`() = runTest {
        ...
        // When
        val loadedP1A1M1Nec = dataSource.loadFingerprintIdentities(
            queryP1A1M1Nec,
            0..10,
            project = project,
            onCandidateLoaded = mockCallback,
        )
        val loadedP1A1M3Nec = dataSource.loadFingerprintIdentities(
            queryP1A1M3Nec,
            0..10,
            project = project,
            onCandidateLoaded = mockCallback,
        )
        val loadedP1A1M2Iso = dataSource.loadFingerprintIdentities(
            queryP1A1M2Iso,
            0..10,
            project = project,
            onCandidateLoaded = mockCallback,
        )
        val loadedP1A2M1NecEmpty = dataSource.loadFingerprintIdentities(
            queryP1A2M1NecEmpty,
            0..10,
            project = project,
            onCandidateLoaded = mockCallback,
