...
import com.simprints.testtools.common.coroutines.createTestChannel
...
class RealmToRoomMigrationWorkerTest {
    ...
    fun `doWork should succeed and skip migration when realm is empty`() = runTest {
        ...
        coVerify(exactly = 0) { realmDataSource.loadAllSubjectsInBatches(any(), any()) }
    ...
    fun `doWork should succeed and migrate data when realm has data`() = runTest {
        ...
        coEvery {
            realmDataSource.loadAllSubjectsInBatches(any(), any())
        } returns createTestChannel(mockSubjectsBatch1, mockSubjectsBatch2)
        ...
        coVerify { realmDataSource.loadAllSubjectsInBatches(RealmToRoomMigrationWorker.BATCH_SIZE, any()) }
        coVerify(exactly = 1) {
    ...
    fun `doWork should fail and increment retry count when migration fails in processRecords`() = runTest {
        ...
        coEvery { realmToRoomMigrationFlagsStore.isDownSyncInProgress() } returns false
        coEvery { realmDataSource.count(any()) } returns 3 // Total subjects
        coEvery { realmDataSource.loadAllSubjectsInBatches(any(), any()) } returns createTestChannel(mockSubjectsBatch1, mockSubjectsBatch2)
