...
class DbSubjectTest {
    ...
    fun fromDbModelToDomain() {
        ...
        with(domainSubject) {
            ...
            assertThat(projectId).isEqualTo(PROJECT_ID)
            assertThat(samples[0].id).isEqualTo(fingerprintSample.id)
            assertThat(samples[0].referenceId).isEqualTo(REFERENCE_ID)
            assertThat(samples[1].referenceId).isEqualTo(REFERENCE_ID)
        }
    }
}
