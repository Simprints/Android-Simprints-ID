package com.simprints.infra.eventsync.sync.commcare.workers
...
import com.simprints.infra.eventsync.sync.commcare.tasks.CommCareEventSyncTask
...
internal class CommCareEventSyncDownloaderWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result = withContext(dispatcher) {
        ...
        try {
            ...
            commCareSyncTask.downSync(this, getDownSyncOperation(), getEventScope(), project).collect {
                count = it.progress
                max = it.maxProgress
    ...
    private fun handleSyncException(t: Throwable) = when (t) {
        ...
        //TODO(milen): This should be handled in UI to show a proper error message
        is SecurityException -> fail(t, t.message, workDataOf(OUTPUT_FAILED_BECAUSE_PERMISSION_REQUIRED to true))
        else -> retry(t)
    }
    ...
    companion object {
        ...
        //TODO(milen): This should probably be extracted to a common file, as it is used in both down workers
