...
internal abstract class BaseEventDownSyncTask(
    ...
    abstract fun handleSyncError(throwable: Throwable): Boolean
    ...
    suspend fun handleSubjectMoveEvent(
        operation: EventDownSyncOperation,
    ...
    fun handleSubjectUpdateEvent(event: EnrolmentRecordUpdateEvent): List<SubjectAction> = with(event.payload) {
        listOf(
