...
internal class EventMigration15to16 : Migration(15, 16) {
    ...
    private fun renameFaceLicenseErrors(database: SupportSQLiteDatabase) {
        ...
        val cursor = database.query(
            "SELECT * FROM $TABLE_NAME WHERE $EVENT_TYPE =?", arrayOf(ALERT_EVENT_TYPE)
        )
        cursor.use {
            while (it.moveToNext()) {
                ...
                if (jsonData?.contains(FACE_LICENSE_MISSING) == true || jsonData?.contains(
                        FACE_LICENSE_INVALID
                    ) == true
                ) {
                    jsonData.let {
                        val updatedJson = jsonData.replace(
