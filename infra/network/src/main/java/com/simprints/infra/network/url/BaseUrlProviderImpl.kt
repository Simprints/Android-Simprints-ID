...
internal class BaseUrlProviderImpl @Inject constructor(
    ...
    @Deprecated("Data has been migrated to secure prefs. Delete after there are no users below 2025.3.0")
    private val prefs: SharedPreferences = context.getSharedPreferences(LEGACY_PREF_FILE_NAME, Context.MODE_PRIVATE)
    ...
    private fun getSecurePrefs(): SharedPreferences {
        if (prefs.contains(API_BASE_URL_KEY)) {
            securePrefs.edit(commit = true) {
                putString(API_BASE_URL_KEY, prefs.getString(API_BASE_URL_KEY, ""))
            }
