...
class RetryHelperKtTest {
    ...
    @Test
    fun `should not retry if there is no exception thrown`() =
    ...
    @Test
    fun `should not retry if the condition is false and throw the exception`() =
        runTest(StandardTestDispatcher()) {
            var nbCall = 0
    ...
    @Test
    fun `should retry if the condition is true and throw the exception at the end`() =
        runTest(StandardTestDispatcher()) {
            var nbCall = 0
            val exception = RuntimeException("exception")
            val thrownException = assertThrows<RuntimeException> {
    ...
    @Test
    fun `should retry until the condition is false and throw the last exception at the end`() =
        runTest(StandardTestDispatcher()) {
            var nbCall = 0
            val exception = RuntimeException("exception")
    ...
    @Test
    fun `should retry until there is no exception thrown`() =
        runTest(StandardTestDispatcher()) {
            var nbCall = 0
            val exception = RuntimeException("exception")
            retryIO(
