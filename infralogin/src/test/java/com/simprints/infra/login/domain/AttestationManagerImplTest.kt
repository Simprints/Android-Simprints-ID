...
class AttestationManagerImplTest {

    companion object {
        private const val NONCE = "nonce"
        private const val NONCE_ENCODED ="bm9uY2U="
        private const val VALID_JWS_RESULT =
    ...
    private val playIntegrityManager = mockk<IntegrityManager>()
    private val attestationResponseTask = mockk<Task<IntegrityTokenResponse>>()
    private val attestationResponse = mockk<IntegrityTokenResponse>()
    private val attestationManagerImpl = AttestationManagerImpl(playIntegrityManager)

    @Before
    fun setup() {
        mockkStatic(Tasks::class)
        every { Tasks.await(attestationResponseTask) } returns attestationResponse
    }

    @Test
    fun `should return the attestation`() = runTest(UnconfinedTestDispatcher()) {
        every {
            playIntegrityManager.requestIntegrityToken(
                IntegrityTokenRequest.builder().setNonce(NONCE_ENCODED).build()
            )
        } returns attestationResponseTask
        every { attestationResponse.token() } returns VALID_JWS_RESULT


        val attestation = attestationManagerImpl.requestPlayIntegrityToken(NONCE)
        assertThat(attestation).isEqualTo(VALID_JWS_RESULT)
    }
