...
/**
 * SourceFinger position on hand.
 * 
 * <p>
 * SourceFinger position is used to speed up matching by skipping fingerprint pairs
 ...
 *   which is default value of {@link SourceFingerprint#setFinger finger} property for new {@link SourceFingerprint} objects.
 * </p>
 * 
 * <p>
 ...
 * </ul>
 * @see SourceFingerprint#setFinger SourceFingerprint.setFinger
 */
public enum SourceFinger
{
    /**
    ...
    private static final Map<Byte,SourceFinger> lookup = new HashMap<Byte,SourceFinger>();

    static {
        for(SourceFinger s : SourceFinger.values())
             lookup.put(s.toByte(), s);
    }

    SourceFinger(int value) {
	 ...
	 */
	public static SourceFinger valueOf(int code) {
        if (code < 0 || code > 10)
